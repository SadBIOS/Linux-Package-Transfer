...
* **Linux Mint Debian Edition<sup>®</sup>** (LMDE 7 Gigi or similar)
...
| `--gen-sys-upgrd-req` | Prepares a full system update request archive | Run on Offline machine |
...
| `--dgst-sys-upgrd-arch <bundle>` | Ingests update metadata or performs system update | Run on Offline machine with fetched bundle |
...
#### Stage B: Performing the Update

1. On the updated **offline machine**, generate the system update request:
...
3. Transfer `depsys-final-TIMESTAMP.tar.gz` to the **offline machine** and execute the update:
